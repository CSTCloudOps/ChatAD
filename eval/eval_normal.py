...
def process_input(model_type: str, processor: Any, prompt: str, image_path: str) -> Dict:
    """
...
def main(TYPE: str = "image"):
    ...
    for item,item_label in zip(df, label):
        ...
        if "Write anomalous intervals (final answer) using python list format in" not in prompt:
            continue
        ...
        generated_ids = model.generate(**inputs, max_new_tokens=512, repetition_penalty=1.1)
