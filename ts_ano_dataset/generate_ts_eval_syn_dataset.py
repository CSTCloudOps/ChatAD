...
def univariate_seed_qa(COT: bool=False):
    ...
    if COT:
        ...
        cur_fields = {"length": "COT", 'label': len(scaled_timeseries)}
        ...
        cur_fields = {"length": "RESULT", 'label': len(scaled_timeseries)}
        fields.append(cur_fields)
    ...
    if COT:
        ...
        cur_fields = {"max": "COT", 'label': round(np.max(scaled_timeseries),3)}
        fields.append(cur_fields)
    else:
        ...
        cur_fields = {"max": "RESULT" , 'label': round(np.max(scaled_timeseries),3)}
        fields.append(cur_fields)
    ...
    if COT:
        ...
        cur_fields = {"min": "COT",'label': round(np.min(scaled_timeseries),3)}
        fields.append(cur_fields)
    else:
        ...
        cur_fields = {"min": "RESULT",'label': round(np.min(scaled_timeseries),3)}
        fields.append(cur_fields)
    ...
    index = [i for i in range(len(scaled_timeseries)) if scaled_timeseries[i]==num]
    if COT:
        ...
        cur_fields = {"index": "COT", 'label': index}
        fields.append(cur_fields)
    else:
        ...
        cur_fields = {"index": "RESULT", 'label': index}
        fields.append(cur_fields)
...
def generate_seed_qa_dataset(COT: bool=False):
    generated_cnt = 0
    ts_idx = 0
    labels = []
    datas = []
    ...
    with tqdm(total=TOTAL_CNT, desc='Generating seed qa') as t, open(OUTPUT_DATASET, 'wt') as f:
        while True:
            ...
            try:
                if random.random() > -1:
                    seed_qa = univariate_seed_qa(COT)
                else:
...
if __name__ == '__main__':
    generate_seed_qa_dataset(False)
