...
# CONFIG
ENCODING_METHOD = 'minmax'
SEQ_LEN = None  # Set to none for random seq_len
TOTAL_CNT = 100000
ANOMALY = True
OUTPUT_DATASET = f'result/template_qa_{TOTAL_CNT}_{ENCODING_METHOD}_{str(ANOMALY)}.json'
OUTPUT_LABEL = f'labels/template_qa_{TOTAL_CNT}_{ENCODING_METHOD}_{str(ANOMALY)}.json'
...
def univariate_seed_qa():
    ...
    # Generate QA
    instruction = f"You are a time series analysis expert. This is a metric called {metric} collected from {category} with length of {current_seq_len}: {cur_ts_prompt + str(list(scaled_timeseries))}. "
